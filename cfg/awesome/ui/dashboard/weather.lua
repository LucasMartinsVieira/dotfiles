...
local api_key = user_vars.weather.key
local coordinates = user_vars.weather.coordinates
...
local hourly_widget = function()
  ...
  widget.update = function(result)
    ...
    time:set_markup(os.date("%H:%M", tonumber(result.dt)))
...
awful.widget.watch(
  ...
  function(_, stdout, stderr)
    if stderr == "" then
      ...
      local description =
        current_weather_widget:get_children_by_id("description")[1]
      local humidity = current_weather_widget:get_children_by_id("humidity")[1]
      local wind = current_weather_widget:get_children_by_id("wind")[1]
      local temp_current =
        current_weather_widget:get_children_by_id("tempareture_current")[1]
      local feels_like =
        current_weather_widget:get_children_by_id("feels_like")[1]
      icon.image = icon_dir
        .. icon_map[result.current.weather[1].icon]
        .. ".svg"
      icon:emit_signal("widget::redraw_needed")
      description:set_text(
        result.current.weather[1].description:gsub("^%l", string.upper)
      )
      humidity:set_text("Humidity: " .. result.current.humidity .. "%")
      wind:set_text("Wind: " .. result.current.wind_speed .. "m/s")
      temp_current:set_markup(
        math.floor(result.current.temp) .. "<sup><span>°</span></sup>"
      )
